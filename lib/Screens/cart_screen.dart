                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Icon(MingCute.shopping_bag_1_fill, color: AppColors.blue, size: 30.h), 
                      
                      SizedBox(width: 5.w), 
                      
                      Text('My Cart', style: GoogleFonts.poppins(fontSize: 22.sp, fontWeight: FontWeight.bold, color: AppColors.customBoldGrey)), 
                      
                      const Spacer(), 
                      
                      IconButton(
                        onPressed: () => BlocProvider.of<CartCubit>(context).clearCart(), 
                  ...
                  BlocBuilder<CartCubit, CartState>(
                    builder: (context, state) 
                    {
                      ...
                      else if (state is CartSuccess) 
                      {
                        ...
                        List<CartModel> filteredCart = myCart.where((cartItem) => cartItem.name.toLowerCase().contains(searchedDrug.toLowerCase())).toList();
              
                        return ListView.separated(
                          shrinkWrap: true,
                          itemCount: filteredCart.length,
                          physics: const NeverScrollableScrollPhysics(),
                          itemBuilder: (context, index) => CustomCartItem(cartModel: filteredCart[index], index: index),
                      ...
                      else 
                      {
                        return Padding(
                          ...
                          child: Row(
                            ...
                            children: [ 
                              Icon(MingCute.shopping_bag_1_fill, color: AppColors.grey, size: 40.h),
                          
                              Text('Empty Cart', style: GoogleFonts.poppins(fontSize: 22.sp, fontWeight: FontWeight.bold, color: AppColors.grey)),
                            ],
                          ),
                        );
                      }
                  ...
                  { 
                    ...
                    child: Padding(
                      ...
                      child: CustomButton(
                        ...
                        buttonBody: Row(
                          children: [
                            Icon(Icons.delivery_dining_rounded, color: AppColors.white, size: 30.h),
                            SizedBox(width: 10.w),
                            Text('Checkout', style: GoogleFonts.poppins(fontSize: 20.sp, fontWeight: FontWeight.bold, color: AppColors.white)),
                            const Spacer(),
                            BlocBuilder<CartCubit, CartState>(
                              builder: (context, state) 
                              {
                                List<CartModel> myCart = BlocProvider.of<CartCubit>(context).cart; 
                                double totalPrice = myCart.fold(0, (sum, item) => sum + (item.price * item.quantity));
                                return Text('${totalPrice.toStringAsFixed(2)} EGP', style: GoogleFonts.poppins(fontSize: 20.sp, fontWeight: FontWeight.bold, color: AppColors.white));
                              },
                            ),
                            SizedBox(width: 10.w),
                            CircleAvatar(
                              backgroundColor: AppColors.white,
                              radius: 15.h,
                              child: Icon(Icons.chevron_right_rounded, color: AppColors.blue, size: 25.h),
                            ),
                          ],
                        ),
                      ),
